  ...
  let navCardsData: {
    ...
    tag: string[];
      ...
      },
      {
        ...
        group: "AI聊天与助手",
        cover: (
          <Image className="rounded-md" width={36} height={36} src="/aivesa.png" alt="1" />
        ),
      ...
      },
      {
        ...
        group: "AI聊天与助手",
        cover: (
          <Image className="rounded-md" width={36} height={36} src="/paper.png" alt="1" />
        ),
      ...
      <aside className="basis-0 grow-[1]">
        <ul className="flex flex-col pt-4 gap-4 text-sm font-semibold">
          {navAsideData.map((item, i) => (
            <li
              key={item.href}
              className={`flex justify-center ${asideIndex === i ? "bg-gradient-to-r from-[#0000ff]" : ""
                }`}
            >
          ...
          <section id={item.title.toLowerCase()} key={item.title}>
            ...
            <div className="pt-4 ">
              <ul className="grid grid-cols-5 grid-rows-2 text-xs gap-4">
                {navCardsData.filter((card) => card.group === item.title).map((card) => (
                  <li key={card.title}>
                    <Link href="/nav">
                      <AICard
                        title={card.title}
